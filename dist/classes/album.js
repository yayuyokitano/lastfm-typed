...
class AlbumClass extends base_1.default {
    ...
    async getInfo(album, params) {
        return await new request_1.default(this.key, this.secret, { method: "album.getInfo", ...album, ...params }).execute();
    }
    async getTags(album, usernameOrSessionKey, params) {
        if (this.isSessionKey(usernameOrSessionKey)) {
            return await new request_1.default(this.key, this.secret, { method: "album.getTags", ...album, sk: usernameOrSessionKey, ...params }).execute();
        }
        else {
            return await new request_1.default(this.key, this.secret, { method: "album.getTags", ...album, user: usernameOrSessionKey, ...params }).execute();
        }
    }
    async getTopTags(album, params) {
        return await new request_1.default(this.key, this.secret, { method: "album.getTopTags", ...album, ...params }).execute();
    }
